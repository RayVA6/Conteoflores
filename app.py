...
# 1. CARGA DEL ARCHIVO (Reemplaza a la ruta fija)
archivo_subido = st.file_uploader("Sube el archivo Excel de Evaluaciones", type=["xlsx", "xls"])

# Caché para que no vuelva a leer las 100k filas cada vez que mueves un filtro
@st.cache_data
...
if archivo_subido is not None:
    ...
    # La magia de Streamlit: Filtramos temporalmente para que el Módulo se adapte al Fundo
    df_temp = df.copy()
    if fundo_sel != 'Todos':
        df_temp = df_temp[df_temp['Nombre Fundo'] == fundo_sel]

    # Filtro Módulo (Sus opciones dependen de df_temp)
    ...
    # Filtro Lotes (Múltiple, depende del Módulo)
